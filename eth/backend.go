...
func New(
	...
	gaslesspool := gaslesspool.New(config.GaslessPool, eth.blockchain)

	eth.txPool, err = txpool.New(config.TxPool.PriceLimit, eth.blockchain, []txpool.SubPool{legacyPool, gaslesspool}) //, blobPool})
	if err != nil {
		return nil, err
	}
