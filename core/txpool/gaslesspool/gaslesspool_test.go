...
func setupPool() (*GaslessPool, *ecdsa.PrivateKey) {
	return setupPoolWithConfig(params.TestChainConfig)
}

func setupPoolWithConfig(config *params.ChainConfig) (*GaslessPool, *ecdsa.PrivateKey) {
...
func validatePoolInternals(pool *GaslessPool) error {
	pool.mu.RLock()
...
func testAddBalance(pool *GaslessPool, addr common.Address, amount *big.Int) {
	pool.mu.Lock()
	pool.currentState.AddBalance(addr, uint256.MustFromBig(amount))
	pool.mu.Unlock()
}

func testSetNonce(pool *GaslessPool, addr common.Address, nonce uint64) {
	pool.mu.Lock()
