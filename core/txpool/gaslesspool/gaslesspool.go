...
// Package gaslesspool implements the normal EVM execution transaction pool.
package gaslesspool
...
// GaslessPool contains all currently known transactions. Transactions
...
func New(config Config, chain BlockChain) *GaslessPool {
...
func (pool *GaslessPool) Init(gasTip uint64, head *types.Header, reserve txpool.AddressReserver) error {
...
func (pool *GaslessPool) Close() error {
...
func (pool *GaslessPool) Reset(oldHead, newHead *types.Header) {
...
func (pool *GaslessPool) SubscribeTransactions(ch chan<- core.NewTxsEvent, reorgs bool) event.Subscription {
...
func (pool *GaslessPool) SetGasTip(tip *big.Int) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	if tip.Cmp(common.Big0) != 0 {
		log.Warn("Gasless pool tip threshold cannot be set to a value other than zero", "tip", tip)
	}

	pool.gasTip.Store(uint256.MustFromBig(common.Big0))
}

func (pool *GaslessPool) SetMinFee(minFee *big.Int) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	if minFee.Cmp(common.Big0) != 0 {
		log.Warn("Gasless pool minimum fee cannot be set to a value other than zero", "minFee", minFee)
	}

	pool.minimumFee = common.Big0
}

// Nonce returns the next nonce of an account, with all transactions executable
// by the pool already applied on top.
func (pool *GaslessPool) Nonce(addr common.Address) uint64 {
	pool.mu.RLock()
...
func (pool *GaslessPool) Stats() (int, int) {
	pool.mu.RLock()
	defer pool.mu.RUnlock()
...
func (pool *GaslessPool) Content() (map[common.Address][]*types.Transaction, map[common.Address][]*types.Transaction) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
func (pool *GaslessPool) ContentFrom(addr common.Address) ([]*types.Transaction, []*types.Transaction) {
	pool.mu.RLock()
	defer pool.mu.RUnlock()
...
func (pool *GaslessPool) IteratePending(f func(tx *types.Transaction) bool) bool {
	pool.mu.RLock()
	defer pool.mu.RUnlock()
...
func (pool *GaslessPool) Locals() []common.Address {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
func (pool *GaslessPool) local() map[common.Address]types.Transactions {
...
func (pool *GaslessPool) validateTxBasics(tx *types.Transaction, local bool) error {
	opts := &txpool.ValidationOptions{
		Config: pool.chainconfig,
		Accept: 0 |
			1<<types.GaslessTxType,
		MaxSize: txMaxSize,
...
func (pool *GaslessPool) validateTx(tx *types.Transaction, local bool) error {
...
func (pool *GaslessPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
...
func (pool *GaslessPool) isGapped(from common.Address, tx *types.Transaction) bool {
...
func (pool *GaslessPool) enqueueTx(hash common.Hash, tx *types.Transaction, local bool, addAll bool) (bool, error) {
...
func (pool *GaslessPool) journalTx(from common.Address, tx *types.Transaction) {
...
//
// Note, this method assumes the pool lock is held!
func (pool *GaslessPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) bool {
...
func (pool *GaslessPool) addLocals(txs []*types.Transaction) []error {
	return pool.Add(txs, !pool.config.NoLocals, true)
}

// addLocal enqueues a single local transaction into the pool if it is valid. This is
// a convenience wrapper around addLocals.
func (pool *GaslessPool) addLocal(tx *types.Transaction) error {
...
func (pool *GaslessPool) addRemotes(txs []*types.Transaction) []error {
	return pool.Add(txs, false, false)
}

// addRemote enqueues a single transaction into the pool if it is valid. This is a convenience
// wrapper around addRemotes.
func (pool *GaslessPool) addRemote(tx *types.Transaction) error {
	return pool.addRemotes([]*types.Transaction{tx})[0]
}

// addRemotesSync is like addRemotes, but waits for pool reorganization. Tests use this method.
func (pool *GaslessPool) addRemotesSync(txs []*types.Transaction) []error {
	return pool.Add(txs, false, true)
}

// This is like addRemotes with a single transaction, but waits for pool reorganization. Tests use this method.
func (pool *GaslessPool) addRemoteSync(tx *types.Transaction) error {
...
func (pool *GaslessPool) Add(txs []*types.Transaction, local, sync bool) []error {
...
func (pool *GaslessPool) addTxsLocked(txs []*types.Transaction, local bool) ([]error, *accountSet) {
...
func (pool *GaslessPool) Status(hash common.Hash) txpool.TxStatus {
	tx := pool.get(hash)
	if tx == nil {
...
func (pool *GaslessPool) Get(hash common.Hash) *types.Transaction {
	tx := pool.get(hash)
	if tx == nil {
		return nil
	}
...
// given hash.
func (pool *GaslessPool) Has(hash common.Hash) bool {
	return pool.all.Get(hash) != nil
}

func (pool *GaslessPool) HasLocal(hash common.Hash) bool {
...
func (pool *GaslessPool) removeTx(hash common.Hash, outofbound bool, unreserve bool) int {
...
func (pool *GaslessPool) requestReset(oldHead *types.Header, newHead *types.Header) chan struct{} {
	select {
...
func (pool *GaslessPool) requestPromoteExecutables(set *accountSet) chan struct{} {
	select {
...
func (pool *GaslessPool) queueTxEvent(tx *types.Transaction) {
	select {
...
func (pool *GaslessPool) scheduleReorgLoop() {
	defer pool.wg.Done()
...
func (pool *GaslessPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*sortedMap) {
...
func (pool *GaslessPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	pool.currentStateLock.Lock()
	defer pool.currentStateLock.Unlock()
...
func (pool *GaslessPool) truncatePending() {
	pending := uint64(0)
	for _, list := range pool.pending {
...
func (pool *GaslessPool) truncateQueue() {
	queued := uint64(0)
	for _, list := range pool.queue {
...
func (pool *GaslessPool) demoteUnexecutables() {
	pool.currentStateLock.Lock()
	defer pool.currentStateLock.Unlock()
...
func (pool *GaslessPool) startPeriodicFeeUpdate() {
...
func (pool *GaslessPool) periodicBaseFeeUpdate() {
	defer pool.wg.Done()
...
func (pool *GaslessPool) updateBaseFee() {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
func (pool *GaslessPool) updateBaseFeeAt(head *types.Header) error {
...
// lookup is used internally by GaslessPool to track transactions while allowing
...
// peeking into the pool in GaslessPool.Get without having to acquire the widely scoped
// GaslessPool.mu mutex.
//
